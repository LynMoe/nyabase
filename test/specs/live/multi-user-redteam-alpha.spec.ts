...
describe('V2 alpha persona live flow', () => {
  it('creates active CPU container, waits actions, exercises stats/power/delete operations, and denies GPU', async () => {
    const state = await loadState();
    const alpha = await loadActor(state, 'alpha');
    const name = `${state.runPrefix}-alpha-v2`;
    const c = await createContainerActive(state, alpha.token, { serverId: state.servers.cpu.id, imageId: state.images.cpuA!.id, name, cpuMillis: 100, memBytes: 64 * 1024 * 1024 });
