...
describe('V2 gamma GPU persona live flow', () => {
  it('creates GPU container when GPU fixture exists and verifies metrics path is non-leaking', async () => {
    ...
    let c;
    try {
      c = await createContainerRunning(state, gamma.token, { serverId: state.servers.gpu.id, imageId: state.images.gpuA.id, name: `${state.runPrefix}-gamma-v2`, cpuMillis: 100, memBytes: 128 * 1024 * 1024, gpuIndices: [0] });
    } catch (error) {
    ...
    await removeContainerViaTask(state, gamma.token, c.id);
  }, 180_000);
});
