...
describe('V2 continuation live flow', () => {
  it('continues after setup with the AgentTask lifecycle', async () => {
    const state = await loadState();
    const alpha = await loadActor(state, 'alpha');
    const c = await createContainerRunning(state, alpha.token, { serverId: state.servers.cpu.id, imageId: state.images.cpuA!.id, name: `${state.runPrefix}-continuation-v2`, cpuMillis: 100, memBytes: 64 * 1024 * 1024 });
